...
const { GET, POST, DELETE, PATCH, expect, defaults } = cds.test(__dirname + '../../', '--with-mocks')
...
describe('Test The GET Endpoints', () => {
  ...
  it('Test Expand Entity Endpoint', async () => {
    const { data } = await GET`/odata/v4/processor/Incidents?$select=title&$top=1`
    expect(data).to.be.an('object')
  })

  it('Should get Orders from Northwind', async () => {
  const { data } = await GET`/odata/v4/processor/getOrders()`
  console.log('Orders received:', JSON.stringify(data, null, 2))
